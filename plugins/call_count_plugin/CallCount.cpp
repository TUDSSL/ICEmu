...
class CallCount : public HookCode {
 ...
 public:
  ...
  CallCount(Emulator &emu) : HookCode(emu, "call_count") {
    ...
    for (auto f : function_names) {
      ...
      cout << printLeader() << " tracking call count for: " << f
           << " at address: 0x" << hex 
           << getEmulator().getArchitecture().getFunctionAddress(func_symbol->address)
           << dec << endl;
  ...
  void run(hook_arg_t *arg) {
    ...
    // TODO: cache the result of getFunctionAddress
    for (auto &f : function_symbols) {
      if (getEmulator().getArchitecture().getFunctionAddress(f->address) == addr) {
        AddressCountMap[f]++;
      }
    }
  }

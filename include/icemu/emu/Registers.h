...
class Registers {
 ...
 private:
  ...
  const std::array<enum reg, 17> regmap = {
      R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, APSR};

 public:
  inline void init(uc_engine *uc) { uc_ = uc; }
  ...
  inline armaddr_t get(int reg) { return get(regmap[reg]); }

  inline void set(enum reg r, armaddr_t reg_val) {
    uc_reg_write(uc_, r, &reg_val);
  }

  inline void set(int reg, armaddr_t val) { set(regmap[reg], val); }
  ...
  void dump(std::ostream &out = std::cout) {
    ...
    for (size_t i = 0; i < regmap.size(); i++) {
      armaddr_t r;
      r = get(i);
      out << "R" << i << ((i < 10) ? " " : "") << ": [0x" << std::setw(w)
          << std::setfill('0') << std::hex << r << "] = " << std::dec << r
          << " " << std::endl;
    }

...
class Pipeline {
 private:
  ...
  vector<armaddr_t> last_instr_immediates;
    ...
    if (cs_arm->cc == ARM_CC_AL) {
      ...
      for (int i=0; i<operand_count; i++) {
        ...
        if (op->type == ARM_OP_IMM) {
          last_instr_immediates.push_back((armaddr_t)op->imm);
        }
      }
    }
    return cycles + P;
...
class CycleCount : public HookCode {
 ...
 public:
  // Always execute
  CycleCount(Emulator &emu) : HookCode(emu, "cycle_count") {
  }

  ~CycleCount() {
    cout << "The program ran for: " << pipeline.cycle_count << " clock cycles (estimate)" << endl;
  }

  void countCycles(armaddr_t address, armaddr_t size) {
    bool ok;
  ...
  void run(hook_arg_t *arg) {
    countCycles(arg->address, arg->size);
  }
};
